...
class returns_dataset(Dataset):
	def __merge(self, df_list: list[pd.DataFrame]) -> list[pd.DataFrame]:
		common_index: pd.Index = reduce(lambda a, b: a.intersection(b), (s.index for s in df_list))
		return [df.loc[common_index] for df in df_list]
	def __init__(self, paths: list[str], target_col: str = ["pred_return"]):
		df_list: list[pd.DataFrame] = self.__merge([pd.read_parquet(path) for path in paths])
